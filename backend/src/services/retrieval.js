...
export async function queryDocument(docId, question, res) {
  try {
    ...
    const vectorStore = documentStores.get(docId);
    if (!vectorStore) {
      throw new Error('Document not found in memory. Please upload it again.');
    }
    ...
    console.log('🔍 Running similarity search in memory...');
    const topChunks = await vectorStore.similaritySearch(question, 10);
